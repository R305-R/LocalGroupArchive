...
assert.match(html, /LOCAL • v0\.9\.3/);
...
assert.match(installer, /PrebuiltDistArchive/);
assert.match(installer, /Resolve-PrebuiltDistArchive/);
assert.match(installer, /Prebuilt Vencord bundle SHA-256 verified/);
assert.match(installer, /patcher\.js", "preload\.js", "renderer\.js", "renderer\.css/);
assert.match(installer, /Platform:\\s\*win32/);
assert.match(installer, /Write-PatcherAsar/);
assert.match(installer, /Test-PatcherAsar/);
assert.match(installer, /_app\.asar/);
...
assert.doesNotMatch(installer, /Install-PnpmForVencord/);
assert.doesNotMatch(installer, /VencordInstallerCli\.exe/);

const remover = fs.readFileSync(new URL("installer/Remove-LocalGroupArchive.ps1", root), "utf8");
assert.match(remover, /Restore-DiscordAppAsar/);
assert.match(remover, /_app\.asar/);
assert.doesNotMatch(remover, /VencordInstallerCli/);
...
assert.match(updater, /\$checksumAssetName = "\$\(\$installerAsset\.name\)\.sha256"/);
...
assert.match(releaseWorkflow, /End-to-end prebuilt installer engine test/);
assert.match(releaseWorkflow, /-SkipInject/);
assert.match(releaseWorkflow, /windows-latest/);
assert.match(releaseWorkflow, /LocalGroupArchive-VencordDist/);
assert.match(releaseWorkflow, /pnpm build --disable-updater/);
